 ...
 */ function groupIntentsByAdapter(intents, allocations) {
    ...
    for (const intent of intents){
        ...
        if (!alloc) {
            throw createValidationFailedError('intent.spec.sourceDomain', intent.spec.sourceDomain, `No allocation found for sourceDomain ${String(intent.spec.sourceDomain)}; ` + 'estimate response contains an intent that does not match any allocation');
        }
    ...
    const provider = {
        ...
        spend: async (params, options)=>{
            ...
            return withEvents('spend', destChainName, {
                ...
                token: params.token ?? 'USDC'
            }, params, async (p)=>spend(p, provider.actionDispatcher, options));
        },
