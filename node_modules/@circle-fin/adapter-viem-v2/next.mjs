...
import { arbitrum, arbitrumSepolia, avalanche, avalancheFuji, base, baseSepolia, celo, celoAlfajores, linea, lineaSepolia, mainnet, optimism, optimismSepolia, polygon, polygonAmoy, sei, seiTestnet, sepolia, sonic, unichain, unichainSepolia, worldchain, worldchainSepolia, xdc, xdcTestnet, zksync, zksyncSepoliaTestnet } from 'viem/chains';
...
function createPrimitive(options) {
    ...
    return async function primitive(input, operation, invocation) {
        ...
        // ─────────────────────────────────────────────────────────────────────────
        const invocationCtx = resolveInvocationContext(invocation, {
...
function createReadAction(name, inputSchema, options, description) {
    ...
    const action = async (input, operation, actionOptions)=>{
        ...
        const ctx = await resolveContexts(operation, adapterContext, actionOptions);
        const execCtx = createExecutionContext({
            name,
            description,
...
/**
 * Maximum number of entries retained in the module-level decimals cache.
 *
 * @remarks
 * Bounds the cache so a long-running, multi-tenant process that bridges many
 * unique tokens cannot grow it without limit. On overflow the oldest
 * (insertion-order) entry is evicted.
 */ const DECIMALS_CACHE_MAX = 1024;
/**
 * Module-level cache keyed by `"<blockchain>:<address>"`.
 *
 * @remarks
 * ERC-20 `decimals()` is immutable once deployed. Caching avoids redundant
 * RPC calls when multiple actions operate on the same token in a session
 * (e.g. `allowance` followed by `approve`). Bounded to
 * {@link DECIMALS_CACHE_MAX} entries with insertion-order (FIFO) eviction.
 */ const decimalsCache = new Map();
/**
 * Insert a decimals value into the bounded cache, evicting the oldest entry
 * when at capacity.
 *
 * @internal
 */ function cacheDecimals(key, decimals) {
    if (decimalsCache.size >= DECIMALS_CACHE_MAX && !decimalsCache.has(key)) {
        const oldest = decimalsCache.keys().next().value;
        if (oldest !== undefined) {
            decimalsCache.delete(oldest);
        }
    }
 ...
 * import type { TokenSelector } from '@core/tokens'
 *
 * async function myBinding(input: { token: TokenSelector }, ctx: EVMTokenContext) {
 ...
 */ async function resolveEVMToken(token, ctx) {
    ...
    if (typeof token === 'object') {
        ...
        const fetchedDecimals = await readDecimals(locator, ctx);
        return {
            locator,
    ...
    if (EVM_ADDRESS_PATTERN.test(token)) {
        ...
        const decimals = await readDecimals(address, ctx);
        return {
            locator: address,
            decimals
        };
...
/** Fetch ERC-20 decimals via the adapter read function, with per-chain caching. */ async function readDecimals(address, ctx) {
    const cacheKey = `${ctx.chain.chain}:${address}`;
    const cached = decimalsCache.get(cacheKey);
    if (cached !== undefined) return cached;
    let raw;
    try {
        // ERC-20 decimals() returns uint8 — ethers/viem decode this as bigint,
        // so coerce to number for Amount compatibility.
        raw = await ctx.read({
            address,
            abi: ERC20_ABI,
        ...
        throw new KitError({
            ...
            // in their message/object graph. Never cache or surface the raw error.
            cause: {
                trace: {
    ...
    // ERC-20 decimals() is a uint8. A non-integer (weird RPC decode → NaN) or
    // out-of-range value (non-conformant token) must not poison the cache.
    const decimals = Number(raw);
    if (!Number.isInteger(decimals) || decimals < 0 || decimals > 255) {
        throw new KitError({
            ...InputError.UNSUPPORTED_TOKEN,
            recoverability: 'FATAL',
            message: `decimals() on ${address} returned an out-of-range value ` + `(${String(raw)}); expected a uint8 (0–255).`
        });
    }
 ...
 *
 * @remarks
 * Mirrors the legacy `assertSignerIsEoa` helper from
 * `@core/adapter-evm/src/actions/gateway/signIntentGroup/assertSignerIsEoa.ts`
 * (PR #954 / DEVX-2774). Internal to the `bindGatewaySignBurnIntents` binding;
 * exported only for direct unit testing of the SCA detection logic.
 *
 * @internal
 */ async function assertSignerIsEoa(readBytecode, address, chain) {
    let code;
    try {
        code = await readBytecode(address, chain);
    } catch (err) {
        // Fail-open: if bytecode cannot be read (RPC down, transient network
        // failure, etc.) downstream signing surfaces its own error. A console
        // warning makes the skip diagnosable without breaking the legacy path.
 ...
 */ function bindGatewaySignBurnIntents(adapter, options = {}) {
    ...
    return gatewayV1SignBurnIntents.bind({
        ...
        execute: async (input, ctx)=>{
            assertEVMChain(ctx.chain);
            // Gateway verifies burn-intent signatures with plain ecrecover and
            // does not fall back to ERC-1271. An SCA signer would silently
            // produce an unverifiable signature and Circle Wallets' KMS rejects
            // the typed data up front with an opaque `<nil>/<nil>` error. When
            // the ecosystem adapter has wired in `readBytecode`, short-circuit
            // with the delegate-pattern recipe before signing. Mirror of the
            // legacy fix landed in #954 (DEVX-2774). When `readBytecode` is
            // omitted (older wirings, mocks, …) the pre-check is skipped — the
            // adapter still produces the same signature it did before this fix.
            const signerAddress = typeof ctx.address === 'string' ? ctx.address : undefined;
            if (readBytecode !== undefined && signerAddress !== undefined && signerAddress.length > 0) {
                await assertSignerIsEoa(readBytecode, signerAddress, ctx.chain);
            }
 ...
 */ /* eslint-disable @typescript-eslint/no-unused-vars -- Stub params required for override signatures. */ /* eslint-disable @typescript-eslint/promise-function-async -- Stubs use Promise.reject/resolve; async/await is unnecessary. */ class LegacyCompatibleAdapterBase extends Adapter {
    ...
    getTokenDecimals(_tokenAddress, _chain) {
        return Promise.reject(new Error('getTokenDecimals() not configured. This is a stub.'));
    }
}
 ...
 * ```
 */ async function adaptWriteAction(action, input, opCtx, enrich) {
    const prepared = await action.prepare(input, opCtx);
    ...
    const legacyMethods = {
        ...
        async prepareAction (action, params, ctx) {
            ...
            if (writeActions.has(action)) {
                return adaptWriteAction(resolved, mapped, ctx, config.enrichWriteRequest);
            }
 ...
 */ function createExecute(ctx, waitForTransaction) {
   ...
   */ async function executeTransaction(input, chain, traceId, signal) {
        ...
        // (optionally) submission — no wallet client is initialized. Otherwise the
        // transaction is signed and broadcast by the wallet client as before.
        const signing = ctx.signing;
        const txId = signing ? await sendViaSigningStrategy(signing, input, chain, signal) : await sendViaWalletClient(input, chain);
   ...
   */ async function sendViaWalletClient(input, chain) {
        const walletClient = await ctx.getWalletClient(chain);
        const viemChain = applyFeeConfig(resolveViemChain(chain), ctx.config?.transaction);
        const txParams = buildTxParams(input.raw, viemChain, input.overrides);
        ...
        if (walletClient.account === undefined) {
            txParams['account'] = input.raw.from;
        }
   ...
   */ async function sendViaSigningStrategy(signing, input, chain, signal) {
        assertStrategySupports(signing, 'evm-calls');
        const fromAddress = input.raw.from;
        const call = {
            to: input.raw.to,
            data: input.raw.data,
        ...
        const result = await signing.execute({
            ...
            fromAddress
        }, buildSigningContext('execute', undefined, signal));
    ...
    const primitive = createPrimitive({
        ...
        execute: async (input, executeCtx)=>{
            return executeTransaction(input, executeCtx.chain, executeCtx.traceId, executeCtx.signal);
        }
    });
    return primitive;
}
        ...
        execute: async (input, executeCtx)=>{
            ...
            if (signing) {
                ...
                const result = await signing.execute({
                    ...
                    fromAddress: address
                }, buildSigningContext('sign-typed-data', input.primaryType, executeCtx.signal));
        ...
        prepare: (input, prepareCtx)=>{
            ...
            return inputs.map((item)=>{
                ...
                return {
                    input: item,
                    built,
                    estimate: async ()=>estimate(built, operationCtx),
                    simulate: async ()=>simulate(built, operationCtx),
...
function createBatchExecute(ctx) {
   ...
   */ async function batchViaSigningStrategy(signing, calls, chain, options) {
        ...
        const result = await signing.execute({
            ...
            calls: payloadCalls,
            fromAddress: sender
        }, buildSigningContext('execute', 'batch', options?.signal));
 ...
 */ function createActionsFromAdapter(deps) {
    return createEVMActions(deps, {
        ...
        // SCA pre-check support for Gateway burn-intent signing. Gateway
        // verifies signatures with ecrecover and does not fall back to
        // ERC-1271, so an SCA signer would silently produce an unverifiable
        // signature. The base binding consumes this callback to throw
        // `INPUT_UNSUPPORTED_ACTION` with the delegate-pattern recipe before
        // signing when bytecode is present at the resolved signer address.
        // Mirror of the legacy fix landed in #954 (DEVX-2774). Non-EVM chains
        // and bytecode-read failures fall through to the legacy behaviour
        // (the binding swallows the error so signing still proceeds).
