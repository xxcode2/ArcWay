 ...
 */ const gatewayTypedDataSchema = zod.z.object({
 ...
 * }
 * ```
 */ const evmTransactionHashSchema = zod.z.string({
 ...
 */ const prepareDepositForBurnWithFees = async (params, adapter, context)=>{
    ...
    // destinationCaller must be the executor on the prepaid FORWARD path; unlike
    // the plain burn it is not optional.
    if (params.destinationCaller === undefined || params.destinationCaller === '') {
        throw createValidationFailedError('destinationCaller', params.destinationCaller, 'destinationCaller is required for depositForBurnWithFees (must be the GenericExecutor)');
    }
    ...
    if (!hexStringSchema.safeParse(signedQuote).success) {
    ...
    if (hasHookData && !hexStringSchema.safeParse(params.hookData).success) {
...
const earnExecuteParamsSchema = zod.z.object({
 ...
 */ const getViemChainByEnum = (chain)=>{
    ...
    switch(definition.chain){
        ...
        case exports.Blockchain.Cronos:
            return chains.cronos;
        case exports.Blockchain.Cronos_Testnet:
            return chains.cronosTestnet;
 ...
 */ class ViemAdapter extends EvmAdapter {
   ...
   */ parseAndValidateAbi(abiInput, functionName) {
        ...
        let abi;
        try {
            abi = Array.isArray(abiInput) && typeof abiInput[0] === 'string' ? viem.parseAbi(abiInput) : abiInput;
        } catch (error) {
   ...
   */ async getTokenDecimals(tokenAddress, chain) {
        try {
            ...
            if (typeof decimalsResult !== 'number' || decimalsResult < 0 || decimalsResult > 255 || !Number.isInteger(decimalsResult)) {
