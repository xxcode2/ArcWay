 ...
 *
 * @internal
 */ const supportedTokenSchema = z.string().transform((v)=>v.trim().toUpperCase()).pipe(z.enum(SUPPORTED_TOKENS));
...
const allocationSchema = z.object({
    amount: z.string().min(1, 'Allocation amount is required.').pipe(createDecimalStringValidator({
        allowZero: false,
        regexMessage: AMOUNT_FORMAT_ERROR_MESSAGE,
        attributeName: 'amount',
        maxDecimals: 6
...
const spendSourceSchema = z.object({
    ...
    allocations: z.union([
        allocationSchema,
        z.array(allocationSchema).min(1)
    ]).optional(),
    address: z.string().optional(),
    sourceAccount: z.string().optional()
}).strict();
const spendDestinationSchema = z.object({
    adapter: adapterSchema,
    chain: unifiedBalanceChainIdentifierSchema,
    recipientAddress: z.string().optional(),
    address: z.string().optional(),
...
const forwarderSpendDestinationSchema = z.object({
    chain: unifiedBalanceChainIdentifierSchema,
...
const customFeeConfigSchema = z.object({
    recipientAddress: z.string().min(1, 'Fee recipient address is required.'),
    value: z.string().min(1, 'Fee value is required.').pipe(createDecimalStringValidator({
        allowZero: false,
        regexMessage: AMOUNT_FORMAT_ERROR_MESSAGE,
        attributeName: 'value',
        maxDecimals: 6
...
const amountSchema = z.string().min(1, 'Amount is required.').pipe(createDecimalStringValidator({
    allowZero: false,
    regexMessage: AMOUNT_FORMAT_ERROR_MESSAGE,
    attributeName: 'amount',
    maxDecimals: 6
...
/** @internal Used by assertSpendParams(). */ const spendParamsSchema = z.object({
...
/** Single chain identifier or non-empty array of identifiers. */ const chainsSchema = z.union([
 ...
 */ const balanceSourceSchema = z.object({
...
/** One source or a non-empty array of sources. */ const sourcesSchema = z.union([
    balanceSourceSchema,
 ...
 * })
 * ```
 */ const getBalancesParamsSchema = z.object({
    token: supportedTokenSchema.optional(),
 ...
 * - `token` — optional, must be a supported token if provided
 */ const delegateParamsBaseSchema = z.object({
...
const removeFundParamsSchema = z.object({
 ...
 */ const initiateRemoveFundParamsSchema = z.object({
    from: adapterContextSchema.strict(),
    amount: z.string().pipe(createDecimalStringValidator({
        allowZero: false,
        regexMessage: AMOUNT_FORMAT_ERROR_MESSAGE,
...
//# sourceMappingURL=unifiedBalance.mjs.map
